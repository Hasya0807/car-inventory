...
export const LoginPage = () => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      addToast(`Successfully logged in as ${role === 'admin' ? 'Dealership Admin' : 'Customer'}!`, 'success');
      navigate('/');
    } catch (err) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-surface p-4">
      <div className="bg-card p-8 rounded-2xl max-w-sm w-full border border-border shadow-xl">
        ...
        <h1 className="text-2xl font-display font-bold text-text-main text-center mb-6">Welcome Back</h1>
